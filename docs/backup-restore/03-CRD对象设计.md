...
统一 `apiVersion: protection.platform.io/v1alpha1`。`metadata.name` 为 DNS-1123；禁止 `metadata.namespace`。平台 API 注入：

```yaml
metadata:
  labels:
    protection.platform.io/cluster: cluster-a
    protection.platform.io/project: project-a
    protection.platform.io/tenant: tenant-a
  annotations:
    protection.platform.io/creator: user-123
    protection.platform.io/creator-type: User
    protection.platform.io/request-id: req-uuid
```

label 用于索引，不替代 spec 授权。控制器写 `status.observedGeneration`；用户不得写 status。时间均 RFC3339 UTC；Duration 使用 Kubernetes duration 字符串。
...
| projectRef | string/是 | `_platform` | Repo 所有者；不可变 |
| tenantRef | string/否 | - | 不可变 |
...
校验：CEL 约束 type 与配置分支、port/连接数范围、SecretRef 完整；Webhook 校验路径、PVC、Secret namespace、known_hosts 与引用删除。不可变：cluster/project/tenant/type、Local mode/path/PVC、SFTP endpoint/basePath/tempSuffix。
...
metadata:
  ...
  labels:
    protection.platform.io/cluster: cluster-a
    protection.platform.io/project: _platform
  annotations:
...
过滤顺序：授权 Namespace 交集 → scope mode → namespace include/exclude → namespaced/cluster GVR include/exclude → Secret/CRD/CR 开关 → labelSelector → 对象级系统排除 → PVC 规则。任一 exclude 优先于 include；资源名在 admission 时由 discovery 规范化为 `plural.group`。
...
| 字段 | 类型/必填 | 默认/规则 | 可变性 |
|---|---|---|---|
| clusterRef/projectRef/tenantRef | string/是/是/否 | 身份 | 不可变 |
| mode | Cluster\|Namespace/是 | - | 不可变 |
| includeNamespaces | []string/条件 | Namespace 模式默认项目授权全集 | 热更新 |
...
| includeClusterResources | bool/否 | Cluster=true, Namespace=false | 项目用户固定 false |
| includeSecrets/includeCRDs/includeCustomResources | bool/否 | false | 权限+加密联校验 |
| labelSelector | metav1.LabelSelector/否 | - | AND；V1.0 单 selector |
| pvc.enabled | bool/否 | true | 是否快照 PVC 数据 |
...
finalizer：`protection.platform.io/scope-protection`；被 enabled Policy 引用时拒删。无 ownerReference。printerColumns：MODE、CLUSTER、PROJECT、PHASE、RESOURCES、PVCS、POLICIES、AGE。

校验：Cluster mode 禁止 includeNamespaces 与项目身份组合；include/exclude 无重复/冲突；label selector 有效；GVR 存在或允许 future CRD 并标 Stale；SnapshotClass driver 匹配。不可变：身份/mode；其余热更新只影响新 Task。
...
|---|---|---|
| identity refs | string | cluster/project/tenant 不可变 |
| scopeRef.name/repositoryRef.name | string/必填 | 可热更新；解析 UID 写 status；同集群/授权 |
...
finalizer：`policy-protection` 仅保证停止调度和解除索引，不级联 Task/Record。无 ownerReference。printerColumns：CRON、TZ、ENABLED、SCOPE、REPO、LAST、NEXT、PHASE、AGE。不可变：身份；name。热更新字段不改已生成 Task。
...
metadata:
  ...
  labels: {protection.platform.io/cluster: cluster-a, protection.platform.io/project: project-a}
spec:
  clusterRef: cluster-a
  projectRef: project-a
  tenantRef: tenant-a
  scopeRef: {name: project-a-production}
...
Task 创建时平台/PolicyController 解析并固化 `scopeSnapshot`、`repositorySnapshot`、授权 Namespace、配置版本和 hash；不在运行中读取可变 Scope/Policy 作为执行真相。

| 字段 | 类型/默认 | 规则 |
|---|---|---|
| cluster/project/tenant | string | 不可变 |
...
| authorizationSnapshot | version,allowedNamespaces,subjectHash | 防止运行中范围漂移；执行前仍检查撤权 |
...
校验/不可变：除 cancelRequested 外 spec 全不可变；Schedule key 唯一由 admission/API 索引保证；用户不能创建伪造的 scopeSnapshot，平台 API 签名或 Controller SA 才可写。
...
kind: BackupTask
metadata:
  name: project-a-daily-20260713t180000z-7c9d
  labels:
    protection.platform.io/cluster: cluster-a
    protection.platform.io/project: project-a
...
spec:
  ...
  tenantRef: tenant-a
  trigger: Schedule
  policyRef: {name: project-a-daily, uid: 44fd9b13-aaaa-bbbb-cccc-333333333333}
  scheduledTime: "2026-07-13T18:00:00Z"
  scopeRef: {name: project-a-production, uid: 1bd8d426-aaaa-bbbb-cccc-111111111111}
  ...
  authorizationSnapshot: {version: authz-rv-902, allowedNamespaces: [app-prod, app-data], subjectHash: sha256:a83d...}
...
| ownership | cluster/project/tenant/creator |
...
spec:
  ...
  taskRef: {name: project-a-daily-20260713t180000z-7c9d, uid: 72c0...}
  policyRef: {name: project-a-daily, uid: 44fd...}
  source:
    clusterRef: cluster-a
    clusterUID: 90af...
  ...
  ownership: {clusterRef: cluster-a, projectRef: project-a, tenantRef: tenant-a, creator: policy/project-a-daily}
status:
  observedGeneration: 1
...
| 字段 | 类型/默认 | 规则/可变性 |
|---|---|---|
| cluster/project/tenant | string | 身份不可变 |
...
| includeClusterResources | bool/false | 项目用户 false |
