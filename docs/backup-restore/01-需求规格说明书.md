...
现有平台缺少统一的 Kubernetes 资源及 PVC 数据保护能力，升级、发布、误操作与存储故障前后依赖人工导出 YAML，存在范围不准确、凭据泄露、任务不可观测、快照与记录失联和恢复顺序错误等风险。本插件将备份目的地、备份范围、调度模板、单次执行、可恢复副本和恢复执行拆成职责明确的集群级对象。
...
| 作用域 | Velero 核心运行对象通常位于 Velero Namespace | 七个核心对象均为 Cluster-scoped | 支持整集群/多 Namespace；不受业务 Namespace 删除影响；恢复可创建 Namespace/集群资源。代价是原生 RBAC 无法做对象级行过滤，当前仅允许管理员使用 |
...
| 角色 | 数据可见范围 | 说明 |
|---|---|---|
| 集群管理员 | 当前 `clusterRef` 下全部对象及全局设置 | 当前发行物唯一受支持的人类操作角色；可管理 Repo/Scope/Policy/Task/Record/Restore |
...
2. `clusterRef` 必填、符合平台集群 ID 规则且创建后不可变；Scope、Repo、Policy、Task、Record、Restore 的关联引用必须属于同一 `clusterRef`。
3. Scope 为 Namespace 时 include 必须非空，exclude 不能与 include 冲突；该校验只保证选择规则有效，不代表调用者获得这些 Namespace 的权限。
...
| BR-SCOPE-001 范围定义 | 集群管理员选择 Cluster/Namespace/GVR/标签 | 集群资源发现 | exclude 优先；资源名规范化为 plural.group；规则可复用；Namespace 列表不是授权清单 | Scope Ready 与 resolved 摘要 | 空范围/GVR 不存在失败 | 预览和实际 Task 的 scope hash 一致 | P0/V1.0 |
...
| BR-SCOPE-003 范围预览 | 集群管理员查看对象估算与风险 | Scope 草稿或已保存 Scope | discovery + 分页 list；最多返回摘要和抽样 | 资源/PVC/Namespace 数、估算字节、警告 | API 限流返回部分结果和 continuation | 预览不写备份；普通用户无预览入口 | P0/V1.0 |
...
| BR-TASK-001 手动/定时备份 | 集群管理员或 Policy 发起一次执行 | Scope/Repo/Policy 或其快照 | 固化资源选择参数，进入完整状态机 | Task、进度、步骤、事件 | 校验/容量/快照/上传失败分类 | 完成后生成独立 Record | P0/V1.0 |
