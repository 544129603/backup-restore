...
|---|---|---|---|---|---|---|
| 概览 | 展示保护覆盖、成功率、容量、风险 | 全部，按范围过滤 | 聚合 API/Prometheus | 查看告警、创建策略、立即备份、发起恢复 | Repo 不可用、任务失败、记录损坏/过期 | Repo/Task/Record 详情 |
| 备份仓库 | 管理副本目的地 | 平台管理员；其他角色只读可用摘要 | Repository API | 新建、编辑、测试、停用、删除 | Ready/Degraded/Unavailable/Deleting | 新建/详情 |
| 备份范围 | 管理资源选择模板 | 平台/集群/项目管理员 | Scope/Discovery/Preview API | 新建、复制、预览、编辑、删除 | Ready/Invalid/Previewing | 预览/策略新建 |
| 备份策略 | 管理调度与保留 | 管理员 | Policy API | 新建、复制、启停、立即执行、删除 | Enabled/Paused/Invalid | Task 列表/新建策略 |
| 备份任务 | 观察单次执行 | 授权用户 | Task/Log API | 查看、取消、重试 | 完整 BackupTask phase | Task 详情/Record 详情 |
| 备份记录 | 管理可恢复资产 | 授权用户 | Record/Check API | 校验、恢复、删除 | Available/Broken/SnapshotMissing/RepoUnavailable/Expired/Deleting | Record 详情/恢复向导 |
| 恢复任务 | 观察恢复执行 | 授权用户 | Restore/Log API | 查看、取消、失败对象导出 | 完整 RestoreTask phase | Restore 详情/Record 详情 |
| 全局设置 | 设置默认、安全、并发、GC、通知 | 平台管理员 | Config API | 编辑、验证、查看生效版本 | Ready/Invalid/Reconciling | 审计/测试通知 |
...
- 前端按钮鉴权仅改善体验，后端始终再校验；收到 403 显示“无权执行该操作”，不推断对象存在性。
- SFTP 密码、私钥、加密密钥永不回显；编辑时仅显示 Secret 引用和“已配置”。known_hosts 可显示指纹摘要。
- 详情页展示 `creator`、创建时间、最近修改人、trace/request ID、关联 project/cluster；不展示 Secret 内容。
- 表格导出最多 10,000 条；日志下载需重新授权，默认脱敏。
...
| clusterRef/projectRef/tenantRef | 从当前上下文选择；项目用户不可切到未授权值 |
...
| 维度 | 设计 |
|---|---|
| 页面目标/角色 | 一屏识别未保护工作负载、失败任务、损坏记录和容量风险；全部角色按授权范围查看 |
| 布局 | 顶部上下文与时间范围；KPI 卡（策略覆盖率、24h 成功率、可用记录、Repo 可用容量）；中部趋势图；底部风险与最近任务 |
| 查询条件 | cluster/project、24h/7d/30d、策略/Repo |
...
| 维度 | 设计 |
|---|---|
| 页面目标/角色 | 查看 Repo 健康、类型、容量和引用；平台管理员管理，其他角色只读可用项 |
...
| 删除确认 | 预检列出 Policy/Record 引用；有记录默认禁止，平台审批后可 orphan；输入 Repo 名称二次确认 |
...
| 维度 | 设计 |
|---|---|
| 页面目标/布局 | 分步：基本信息→类型配置→安全/性能→连通性测试→确认；平台管理员 |
...
| 维度 | 设计 |
|---|---|
| 目标/角色 | 管理可复用选择规则；平台/集群/项目管理员 |
| 查询/字段 | cluster/project、mode、phase、hasSnapshots、关键字；名称、模式、Namespace 摘要、GVR 摘要、标签、PVC、预估对象数、策略数、最近预览 |
| 操作 | 新建、复制、编辑、预览、基于范围建策略、删除 |
| 可用条件 | Scope Ready 才可建策略；被启用 Policy 引用时不可删；项目用户只见本项目 |
...
| 基本字段 | name、clusterRef、projectRef、mode=`Cluster|Namespace`、includeNamespaces、excludeNamespaces、allowedNamespaces（只读） |
...
| 校验/规则 | exclude 优先；include `*` 不与显式值混用；项目用户不能 Cluster；includeCR=false 时集群资源选择禁用；Hook enabled 在 V1.0 拒绝 |
| 默认 | Namespace 用户默认当前项目全部 Namespace；默认排除运行时资源；Secret/CRD/CR false；PVC true；CrashConsistent；快照失败=Fail |
...
| 校验/默认 | Preview TTL 10m；提交时校验 scope hash 和授权版本未变 |
| 错误/空/加载 | 分页错误保留已统计数据；0 资源说明过滤命中；进度显示 discovery/list/pvc-check |
| 权限/跳转/API | 对象名仅授权用户可见；`POST /scope-previews`、`GET /scope-previews/{id}` |

### 3.8 备份策略列表

| 维度 | 设计 |
|---|---|
| 目标/查询 | 管理 RPO；cluster/project、enabled、health、Repo、Scope、trigger window |
...
| 字段 | name、cluster/project、scopeRef、repositoryRef、cron、timezone、enabled、suspend、concurrencyPolicy、missedRunPolicy/window/maxCatchUpRuns、retention.count/age、snapshotLifecycle、retry、timeout、notification |
| 校验 | 引用同 cluster/project 或共享授权；count 1–10,000 或空，age 1h–3650d；Forbid/Replace 规则明确；RunAll maxCatchUpRuns ≤10 |
| 默认 | enabled=false（创建后确认开启）；Forbid；Skip；count=7、age=30d；retry 3；timeout 4h；Etc/UTC |
| 操作 | 保存草稿、保存并启用、预览未来 5 次、复制；立即执行走独立确认，不修改策略 |
| 错误/加载/权限 | DST 重复/跳过在预览标识；无可用 Repo/Scope 显示前置入口；项目角色无法选 Cluster Scope |
...
| 维度 | 设计 |
|---|---|
| 目标/查询 | 观察执行；cluster/project、trigger、policy、phase、时间、errorCode |
...
| 维度 | 设计 |
|---|---|
| 目标/查询 | 选择可靠恢复点；cluster/project/repo/availability、策略、时间、到期、含 PVC |
...
| 删除确认 | 选择 CROnly/Data/DataAndSnapshots，列出精确影响；输入记录名；高危权限和短期 token |
...
| 删除/权限/API | 三模式二次确认；详情信息按角色过滤；`GET /backup-records/{name}`、resources、snapshots、restores、`:verify/:delete` |
...
| 2 目标集群 | V1.0 固定 source cluster；模式 Original/New/Mapping | 跨集群选项禁用并说明 V2.0；校验当前权限和版本 | `POST /restore-prechecks/target` |
| 3 Namespace 映射 | source→target；未映射默认同名 | 目标 DNS-1123、不得重复汇聚（除管理员显式允许）、必须在 allowedNamespaces 或具有 create project namespace 权限 | `POST /restore-prechecks/namespaces` |
| 4 资源 | GVR 树、对象选择、includeClusterResources、metadataOnly | 默认记录内全部允许资源；依赖自动勾选并可解释；项目用户隐藏集群资源 | `GET /backup-records/{id}/resources` |
...
| 8 确认 | task name、timeout、failurePolicy、确认声明 | planHash/recordUID/authorizationVersion 未变；集群级/Overwrite 输入目标集群名；提交后 spec 不可改 | `POST /restore-tasks` |
...
| 维度 | 设计 |
|---|---|
| 目标/查询 | 查看恢复结果；cluster/project、record、phase、目标 Namespace、时间 |
...
| 空/加载/权限 | 无任务引导从 Record 发起；只展示当前授权目标；轮询规则同 Task |
...
| 权限/二次确认/API | 计划仅授权用户可见；取消确认残留影响；重试需重新确认高危策略；`GET /restore-tasks/{id}`、resources、logs、`:cancel/:retry` |

### 3.17 全局设置

| 维度 | 设计 |
|---|---|
| 目标/角色 | 平台管理员设置控制器默认、安全边界、配额、GC/告警 |
...
5. 任一错误码 → 详情错误 Tab；平台管理员可跳到 Repo/Scope/Config 关联对象。
