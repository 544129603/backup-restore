...
| 校验/默认 | Preview 每 10m 刷新并写入 `status.selectionPreview`；Task 固化 selectionSnapshot；未来外部 ACL 上线后还必须校验授权版本 |
...
| 维度 | 设计 |
|---|---|
| 入口 | 策略列表/详情“立即执行”，或执行历史“新建任务” |
| 字段 | task name、policyRef、timeout、failurePolicy、allowPartialRecord；不再填写 selection 或 repositoryRef |
| 固化 | Controller 在 Pending 阶段解析 Policy，写入 policy UID/generation、repositoryRef/generation 和 selectionSnapshot |
| 操作/跳转 | 二次确认后创建独立 Manual Task，立即跳到执行详情；不修改策略调度 |
...
2. Policy “立即执行” → 创建仅引用 `policyRef` 的 Manual Task → 执行详情。
3. BackupTask `Completed/PartiallyFailed` 且产生 Record → 恢复点详情；无 Record 时显示“未形成恢复点”。
