...
flowchart TB
  ...
  subgraph OP["Backup Restore Operator"]
    ...
    MGR --> RC & SC & PC & BTC & BRC & RTC & RET & GC & CFG
  end
  KAPI <--> OP
...
| ScopeController | BackupScope；API discovery 变化、VSC/SC/CSIDriver | spec、预览 action、能力变化、delete | 规范化过滤、SSAR、分页预览、snapshot capability；`scopeUID/generation/discoveryHash` | 429/5xx 重试；Invalid 不热循环；scope-protection |
| PolicyController | BackupPolicy；Repo/Scope condition；BackupTask 反向索引；时钟 requeue | spec/引用状态/nextRun/Task 终态/delete | 计算 due schedules、并发/补偿、创建 Task；`policyUID/scheduledTimeUTC` | create conflict 后读回；计算错误 Invalid；policy-protection 不级联 |
...
使用 field indexer：Policy by scope/repo UID；Task by policy UID/phase/schedule key；Record by repo/policy/task UID/availability/expiry；Restore by record UID/phase；VolumeSnapshot by task/record UID label。Secret watch 只入队引用它的 Repo/Config，不缓存 data、不把 Secret 值写事件。

集群 discovery 没有稳定原生 watch，ScopeController 每 10 分钟计算 APIResource hash，CRD/StorageClass/VolumeSnapshotClass/CSIDriver 事件触发提前刷新。
...
| 组件 | 输入/输出 | 关键规则 |
|---|---|---|
| ResourceCollector | Scope snapshot→流式 unstructured objects | discovery+分页 List；RV 记录在 index；API QPS/burst 限流；单对象上限 |
...
| Validating | 获得执行槽 | 校验 refs UID、授权未撤销、Repo/Scope/config、timeout；Validated | Preparing | Failed | 瞬态可重试；可取消 |
