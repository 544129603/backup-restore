...
- 用户只能调用平台 API，普通租户不直接 list/watch Cluster-scoped CRD。
- 认证沿用平台 Bearer/OIDC；授权动作采用 `backup.repository.manage`、`backup.scope.manage`、`backup.policy.manage`、`backup.execute`、`backup.record.delete`、`restore.execute` 等。
...
查询参数：`clusterRef` 必填（平台管理员全局聚合接口除外）、`projectRef` 可选但普通用户由上下文强制、`tenantRef` 可选。服务端忽略/拒绝客户端越权扩大范围。
...
```json
{
  ...
  "error": {
    "httpStatus": 422,
    "code": "BR-SCOPE-INVALID-001",
    "message": "备份范围没有匹配到可备份资源",
    "retryable": false,
    "field": "spec.includeNamespaces",
    "details": [{"namespace": "app-a", "reason": "NotAllowed"}],
    "help": "请选择当前项目已授权的命名空间"
  }
}
```
...
|---|---|---|---|---|
| GET `/repositories` | cluster/project,type,phase,enabled,keyword,pageSize,continue | Repo 脱敏摘要列表 | repository.read | 403、BR-CLUSTER-NOTFOUND-002 |
...
```json
{
  ...
  "spec": {
    "clusterRef": "cluster-a", "projectRef": "_platform", "type": "SFTP", "enabled": true,
    "sftp": {
...
## 3. Scope 与 Preview API

| Method/Path | 参数/请求体 | 返回 | 权限 | 典型错误 |
|---|---|---|---|---|
| GET `/scopes` | cluster/project,mode,phase,hasPVC,keyword,paging | Scope 摘要 | scope.read | 403 |
| POST `/scopes` | metadata+BackupScope spec+previewHash(建议) | 201 Scope | scope.manage | BR-SCOPE-INVALID-001、BR-PERMISSION-DENIED-001 |
| GET `/scopes/{name}` | - | Scope 详情 | scope.read | 404 |
| PUT `/scopes/{name}` | resourceVersion+可变字段 | 200 Scope | scope.manage | 409、immutable |
| POST `/scopes/{name}:clone` | newName,targetProjectRef | 201 disabled/Ready draft | scope.manage | 422 越权 namespace |
| POST `/scopes/{name}:preview` | optional overrides | 202 Preview | scope.preview | discovery/429 |
| POST `/scope-previews` | 完整 Scope draft、`sampleLimit`≤100 | 202 Preview | scope.preview | BR-SCOPE-INVALID-001 |
| GET `/scope-previews/{id}` | page/GVR/namespace/filter | Preview 状态和分页结果 | preview owner/read | 404/410 expired |
| DELETE `/scopes/{name}` | resourceVersion,confirmName | 202 | scope.delete | BR-SCOPE-INUSE-002 |
| GET `/clusters/{cluster}/api-resources` | scope=Namespaced|Cluster,search | 规范 GVR、kind、verbs、category | scope.read | 503 discovery |
| GET `/clusters/{cluster}/snapshot-capabilities` | namespace/storageClass/driver | CSI/VSC 支持矩阵 | scope.read | snapshot API absent |
...
{
  "scope": {
    "clusterRef": "cluster-a", "projectRef": "project-a", "mode": "Namespace",
...
|---|---|---|---|
| GET `/policies` | 筛选 cluster/project/enabled/phase/repo/scope；返回分页摘要 | policy.read | 403 |
...
| POST `/policies/{name}:enable|disable|suspend|resume` | resourceVersion,reason,`skipImmediate`（resume 可选） | policy.manage | Repo/Scope not ready |
...
| Method/Path | 请求/返回 | 权限 | 错误 |
|---|---|---|---|
| GET `/backup-tasks` | cluster/project,trigger,policy,phase,errorCode,start/end,paging | task.read | 403 |
| POST `/backup-tasks` | policyRef 或 scopeRef+repositoryRef、timeout/failurePolicy；202 | backup.execute | ref/permission/capacity |
...
```json
{
  ...
  "spec": {
    "clusterRef": "cluster-a", "projectRef": "_platform",
    "scopeRef": {"name": "cluster-full"},
    "repositoryRef": {"name": "sftp-prod"},
    "timeout": "4h", "failurePolicy": "FailFast"
  }
}
```
...
| Method/Path | 请求/返回 | 权限 | 错误 |
|---|---|---|---|
| GET `/backup-records` | cluster/project/repo/availability/policy/time/containsPVC/paging | record.read | 403 |
...
| POST `/restore-prechecks/target` | recordRef,targetClusterRef | 目标版本/权限/同集群结果 | restore.plan | BR-RESTORE-CROSSCLUSTER-004 |
| POST `/restore-prechecks/namespaces` | mapping | namespace/配额/授权结果 | restore.plan | mapping conflict |
...
| GET `/restore-tasks` | cluster/project/record/phase/ns/time/paging | 列表 | restore.read | 403 |
...
计划 issue：`severity=Blocking|Warning|Info`、code、objectRef、source/target diff 摘要、suggestedAction。响应不返回 Secret value。提交 RestoreTask 必须原样携带 `planId/planHash`，服务端再次核验 Record checksum、cluster UID、discovery hash、授权版本和 TTL。`:retry` 不是复活旧任务：它复用选择参数、重新预检并创建 `trigger=Retry,parentTaskRef=<old>` 的新任务；若预检出现 Blocking，返回 409 和新 planId 供向导处理。
...
| GET `/overview` | timeRange,cluster/project；KPI/趋势/风险 | overview.read |
...
审计记录至少包含 actor/subject、delegated service account、action、object name/UID、cluster/project、before/after 字段摘要（SecretRef 仅元数据）、result/errorCode、requestId/traceId、source IP、time。审计不随业务 CR 删除。

## 9. Watch/事件推送

平台 API 可提供 `GET /events/stream?resourceType=BackupTask&resourceName=...` SSE。服务端从内部 watch 转换为授权过滤后的 domain event，不透传全局 CRD watch。客户端必须处理 `event: reset` 后重新 GET；`Last-Event-ID` 最多保留 10 分钟。无 SSE 时使用 ETag/`resourceVersion` 条件轮询。
...
| BR-SCOPE-INVALID-001 | ScopeInvalidOrEmpty | 备份范围无效或未匹配资源 | include/exclude、GVR discovery、授权 Namespace | 否 |
| BR-SCOPE-INUSE-002 | ScopeInUse | 范围被已启用策略引用 | 查询 Policy，停用/切换引用 | 否 |
| BR-SCOPE-PREVIEW-003 | PreviewPartial | 范围预览不完整 | API 429/timeout/GVR permission，按 continuation 重试 | 是 |
...
| BR-PERMISSION-DENIED-001 | PermissionDenied | 无权访问或执行该操作 | IAM role、project/namespace mapping、RBAC、Webhook audit | 授权后是 |
