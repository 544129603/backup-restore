...
- 管理员 API 可沿用平台 Bearer/OIDC；动作采用 `backup.repository.manage`、`backup.scope.manage`、`backup.policy.manage`、`backup.execute`、`backup.record.delete`、`restore.execute` 等。未来普通用户接入还必须增加独立的对象与 Namespace ACL，不得仅复用这些动作名。
...
| Method/Path | 参数/请求体 | 返回 | 权限 | 典型错误 |
|---|---|---|---|---|
| GET `/scopes` | cluster,mode,phase,hasPVC,keyword,paging | Scope 摘要 | scope.read | 403 |
...
| POST `/scopes/{name}:clone` | newName | 201 disabled/Ready draft | scope.manage | 422 无效 Namespace/过滤规则 |
...
{
  "scope": {
    "clusterRef": "cluster-a", "mode": "Namespace",
...
|---|---|---|---|
| GET `/policies` | 筛选 cluster/enabled/phase/repo/scope；返回分页摘要 | policy.read | 403 |
...
| BR-SCOPE-INVALID-001 | ScopeInvalidOrEmpty | 备份范围无效或未匹配资源 | include/exclude、GVR discovery、Namespace 是否存在 | 否 |
